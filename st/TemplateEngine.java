...
public class TemplateEngine {
    ...
    // new variables
    private Boolean containsYear = Boolean.FALSE;
    private Boolean containsBaseYear = Boolean.FALSE;
    public static final Integer CURRENT_YEAR = 2018;
    private String baseYearValue = null;
    ...
    // identify templates in the template string
            ...
            if (Character.compare(templateString.charAt(charIndex), TEMPLATE_END) == 0){
                if (!templateCandidates.isEmpty()){
                    ...
                    if ((startIndex + 1) == charIndex){
                    	...
                    	// check whether 'year' is specified to determine whether to apply special rules
                    	String sub =  templateString.substring(startIndex+1, charIndex);
                    	if (sub.equals("year")){
                    		containsYear = true;
                    	}
    ...
    // replace string with value of template
    private Result instantiate(String instancedString, ArrayList<Template> sortedTemplates, ArrayList<EntryMap.Entry> sortedEntries, Integer matchingMode){
        ...
        for (Integer i=0; i<sortedTemplates.size(); i++){
            ...
            // check whether base_year is inside sortedEntries
            for (int k = 0; k < sortedEntries.size(); k++){
            	if (sortedEntries.get(k).getPattern().equals("base_year")){
            		containsBaseYear = true;
            		baseYearValue = sortedEntries.get(k).getValue();
            	}
            	if (sortedEntries.get(k).getPattern().equals("year")){
            		containsYear = true;
            	}
            }
            ...
            for(Integer j=0; j<sortedEntries.size(); j++){			
                ...
                // if map contains template
                if (isAMatch(currentTemplate, currentEntry, matchingMode)){
                	// replace ${template} in string with value of template
                	String value = currentEntry.getValue();
                    instancedString = doReplace(instancedString, currentTemplate, i, value, sortedTemplates);
    ...
    // edit value of map when ${year} occurs
    private String doReplace(String instancedString, Template currentTemplate, Integer currentTemplateIndex, String replaceValue, ArrayList<Template> sortedTemplates){
        ...
        // check whether value is "X years ago" or "in X years"
        int year = CURRENT_YEAR;
        if (containsYear){
	        String[] valWords = replaceValue.split(" ");
	        int X = 0;
	        if (valWords.length == 3){		
	        	if (containsBaseYear){									// if map contains the pattern "base_year"
	        		year = Integer.parseInt(baseYearValue);				// update year to base_year rather than CURRENT_YEAR (default)
	            }
	        	boolean cond1 = valWords[1].equals("years") && valWords[2].equals("ago");
	        	boolean cond2 = valWords[0].equals("in") && valWords[2].equals("years");
	        	// years ago, subtract X from current year
	        	if (cond1){
	        		X = Integer.parseInt(valWords[0]);
	        		if (X >= 0){
	        			replaceValue = Integer.toString(year - X);
	        		}
	        	}
	        	// in X years, add X to current year
	        	else if (cond2){
	        		X = Integer.parseInt(valWords[1]);
	        		if (X >= 0){
	        			replaceValue = Integer.toString(year + X);
	        		}
	        	}
	        }
	        else if (valWords.length == 1 && containsBaseYear){
	        	replaceValue = valWords[0];
	        }
        }
